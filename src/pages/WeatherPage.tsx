...
// importa JSON (asegúrate de tener "resolveJsonModule": true en tsconfig)
...
type City = {
  ...
  hourly: Array<{ label: string; icon?: string; temp: number }>
...
export default function WeatherPage() {
  ...
  const [selectedCityId, setSelectedCityId] = useState<string>(() => cities[0]?.id ?? 'cancun')
  const [city, setCity] = useState<City | null>(() => cities.find((c) => c.id === selectedCityId) ?? null)
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center">
        ...
        <div className="w-full max-w-[520px] px-4 sm:px-0 mb-4 flex justify-end">
          <select
            ...
            className="px-3 py-2 rounded-full bg-[var(--panel)] text-[var(--dark)] outline-none"
      ...
      <aside className="flex flex-col items-center gap-8 w-full lg:w-1/2 max-w-[520px]">
        <div className="flex gap-4">
          <button
            ...
            className={`h-10 px-6 rounded-full font-semibold transition-all duration-300 ${viewMode === 'hoy'
          ...
          <button
            ...
            className={`h-10 px-6 rounded-full font-semibold transition-all duration-300 ${viewMode === 'semana'
              ? 'bg-[var(--accent)] text-white shadow-michi-1'
              : 'bg-[var(--panel)] text-[var(--dark)] dark:text-[var(--white)] hover:bg-[var(--accent)]/20'}`}
          >
            Semana
